...
def myserver():
    while True:
        ...
        try:
            ...
            print "Tidak bisa terkoneksikan, error %s" %(err)
            sys.exit()
    
        str = raw_input("Masukkan perintah, ketik 'help' untuk bantuan: ")
        str = str.split()    
        ...
        if(str[0].upper() == "LS"):
            ...
            s.send(json.dumps(js))
            print "-----------------------------------------------------"
            print "Perintah yang tersedia : ls, get, put, rm, quit, help"
            print "-----------------------------------------------------"
